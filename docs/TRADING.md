...
This document contains trading-specific invariants. For general runtime engine invariants, see `docs/ALGO.md`.
...
Trading invariants use the `T` prefix (T1, T2a, etc.) to avoid collision with engine invariants in `docs/ALGO.md`.

---

## Strategy Invariants

### T1. Strategy ID Uniqueness
...
### T2. Broker Configuration Validity
- **[EXEC]** 
...
### T10. Order ID Uniqueness
- Order IDs are UUID v4 strings.
- Unique within a strategy.
- **[EXEC]** Enforced at creation time.

### T11. Order Lifecycle Well-Formed
...
### T12. Order Belongs to One Strategy
- **[REPLAY]** Enforced during replay.
- `Order.StrategyID` must match a valid StrategyID.

### T13. Order Symbol Validity
...
### T16. Order Quantity Constraints
- **[EXEC]** Enforced before `order.submitted`.
...
### T20. Position Quantity Validity
...
### T22. Position Belongs to One Strategy
- **[REPLAY]** Enforced during replay.
- `Position.StrategyID` must match a valid StrategyID.

### T23. Position Symbol Uniqueness
...
### T30. Bar Timestamp Monotonicity
- **[REPLAY]** Enforced during replay.
...
### T32. Market Data Symbol Validity
...
### T40. Signal ID Uniqueness
- Signal IDs are UUID v4 strings.
- Unique within a strategy.
- **[EXEC]** Enforced at creation time.

### T41. Signal Lifecycle
...
### T62. Order Acknowledgment Timeout
...
### T82. LLM Request/Response Lifecycle
...
### T90. Mode Configuration Validity
...
### T91. Approval Queue Management (Mode 1)
...
### T93. Autonomous Mode Safeguards (Mode 2)
- **[EXEC]** Even in autonomous mode, hard limits apply:
  - Daily loss limit (Invariant T53)
  - Position size limit (Invariant T50)
  - Buying power check (Invariant T51)
  - Maximum orders per hour (`max_orders_per_hour_autonomous`)
- Autonomous mode pauses (not fails) if limits approached, resumes when LLM asks for permission.
...
### T96. Mode Switching Prohibition
...
### T104. Duplicate Action Prevention
...
This document defines trading-domain invariants for the AI-powered algorithmic trading platform. For general runtime engine invariants (runs, steps, phases, event log), see `docs/ALGO.md`.

**Precedence:** When rules conflict between docs, `docs/ALGO.md` wins for engine-level constraints; `docs/TRADING.md` (this file) covers trading-specific rules.

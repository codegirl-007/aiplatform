...
1. Create an E*TRADE developer account at https://developer.etrade.com/
2. Navigate to "My Keys" and create a new application
3. Copy your **Consumer Key** and **Consumer Secret** for sandbox environment
...
The E*TRADE API uses OAuth 1.0a with an **out-of-band (OOB)** verification flow:

1. **Request token**: Application exchanges consumer key/secret for a temporary request token
2. **User authorization**: Application opens E*TRADE authorization page in browser
3. **Verification code**: User logs in to E*TRADE and copies the 5-character verification code
4. **Access token**: Application exchanges request token + verification code for access token
5. **Token persistence**: Access token is saved to `{workspace}/.aiplatform/credentials/etrade_tokens.json`
...
E*TRADE access tokens expire:
- At **midnight US Eastern time** (daily)
- After **2 hours of inactivity**

When token expires, run `go run ./cmd/etrade-oauth-test` again to re-authenticate.

### Token Storage

**Current implementation** (COD-14):
- Tokens stored in `{workspace}/.aiplatform/credentials/etrade_tokens.json`
- Suitable for development/testing

**Future implementation** (COD-16):
- Production release will use OS-appropriate secure storage:
  - macOS: Keychain
...
The OAuth implementation is in `internals/clients/etrade_oauth.go`:

```go
// Create OAuth config
config := NewOAuthConfig(consumerKey, consumerSecret, sandbox)

// Step 1: Request token
requestToken, requestSecret, err := RequestToken(config)

// Step 2: Build authorization URL
authURL := AuthorizationURL(consumerKey, requestToken)

// Step 3: User opens authURL, gets verification code

// Step 4: Exchange for access token
accessToken, accessSecret, err := ExchangeToken(config, requestToken, requestSecret, verifier)

// Step 5: Create OAuth-signed HTTP client
client := NewOAuthClient(config, accessToken, accessSecret)
```

### API Endpoints

**Sandbox** (default):
- OAuth: `https://apisb.etrade.com/oauth/*`
- API: `https://apisb.etrade.com/v1/*`

**Production** (set `ETRADE_SANDBOX=false`):
- OAuth: `https://api.etrade.com/oauth/*`
- API: `https://api.etrade.com/v1/*`
